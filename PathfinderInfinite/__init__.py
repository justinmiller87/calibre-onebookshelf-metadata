...
class ConfigWidget(QWidget):
    def __init__(self, plugin):
        ...
        # Cookie Input
        self.cookie_label = QLabel('Pathfinder Infinite "cf_clearance" Cookie:', self)
        layout.addWidget(self.cookie_label)
        
        self.cookie_input = QLineEdit(self)
        self.cookie_input.setText(plugin.prefs.get('cf_cookie_pfi', ''))
        self.cookie_input.setPlaceholderText('Paste Pathfinder Infinite cookie here')
        layout.addWidget(self.cookie_input)

        layout.addStretch(1)

    def save_settings(self):
        self.plugin.prefs['cf_cookie_pfi'] = self.cookie_input.text().strip()

class PathfinderInfiniteSource(Source):
    name = 'Pathfinder Infinite Metadata'
    description = 'Downloads metadata and covers from Pathfinder Infinite.'
    ...
    touched_fields = frozenset(['title', 'authors', 'tags', 'pubdate', 'comments', 'publisher', 'identifier:pathfinderinfinite', 'series', 'rating'])
    ...
    def get_base_url(self):
        return 'https://www.pathfinderinfinite.com'
    ...
    def _fetch_url(self, url, timeout=30):
        ...
        # Select cookie
        cookie = self.prefs.get('cf_cookie_pfi')
    ...
    def identify(self, log, result_queue, abort, title=None, authors=None, identifiers={}, timeout=30):
        ...
        plugin_id = identifiers.get('pathfinderinfinite', None)
    ...
    def _api_search(self, query, timeout, log):
        import json
        # api.pathfinderinfinite.com/api/vBeta/search_ahead?keyword=...
        base = "https://api.pathfinderinfinite.com/api/vBeta/search_ahead"
        
        # Site ID: 78, Group ID: 31
        site_id = 78
        group_id = 31
    ...
    def _get_product_details(self, product_id, result_queue, log, timeout):
        ...
        url = f"https://api.pathfinderinfinite.com/api/vBeta/products/{product_id}"
        ...
        mi.set_identifier('pathfinderinfinite', str(product_id))
        mi.source = self.name
        ...
        if img_path:
            # Construct full URL
            cover_url = f"https://www.pathfinderinfinite.com/images/{img_path}"
    ...
    def get_cached_cover_url(self, identifiers):
        ...
        plugin_id = identifiers.get('pathfinderinfinite')
        if plugin_id:
...
if __name__ == '__main__':
    ...
    import os
    if os.path.exists('../cookie_pfi.txt'):
        with open('../cookie_pfi.txt', 'r') as f:
            cookie = f.read().strip()
            print(f"Using PFI cookie from file: {cookie[:10]}...")
            PathfinderInfiniteSource.prefs['cf_cookie_pfi'] = cookie
            
    print("Running Pathfinder Infinite tests...")
    tests = [
        (
            {'identifiers': {},
             'title': 'Pathfinder', 'authors': ['Paizo']},
             # Structural test
            [title_test('Pathfinder', exact=False)]
        )
    ]
    
    test_identify_plugin(PathfinderInfiniteSource.name, tests)
