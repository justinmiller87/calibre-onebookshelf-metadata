...
class DMsGuildSource(Source):
    name = 'DMsGuild Metadata'
    description = 'Downloads metadata and covers from DMsGuild.'
    ...
    touched_fields = frozenset(['title', 'authors', 'tags', 'pubdate', 'comments', 'publisher', 'identifier:dmsguild', 'series', 'rating'])
    ...
    def get_cached_cover_url(self, identifiers):
        ...
        if dmsguild_id:
            return getattr(self, '_cached_cover', {}).get(dmsguild_id)
        return None
...
if __name__ == '__main__':
    ...
    if os.path.exists('../cookie_dmg.txt'):
        with open('../cookie_dmg.txt', 'r') as f:
            ...
            DMsGuildSource.prefs['cf_cookie_dmg'] = cookie
            
    print("Running DMsGuild tests...")
    tests = [
        (
            {'identifiers': {},
    ...
    test_identify_plugin(DMsGuildSource.name, tests)
