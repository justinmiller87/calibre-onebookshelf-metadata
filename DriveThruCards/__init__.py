...
class ConfigWidget(QWidget):
    def __init__(self, plugin):
        ...
        # Cookie Input
        self.cookie_label = QLabel('DriveThruCards "cf_clearance" Cookie:', self)
        layout.addWidget(self.cookie_label)
        
        self.cookie_input = QLineEdit(self)
        self.cookie_input.setText(plugin.prefs.get('cf_cookie_dtcards', ''))
        self.cookie_input.setPlaceholderText('Paste DriveThruCards cookie here')
        layout.addWidget(self.cookie_input)

        layout.addStretch(1)

    def save_settings(self):
        self.plugin.prefs['cf_cookie_dtcards'] = self.cookie_input.text().strip()

class DriveThruCardsSource(Source):
    name = 'DriveThruCards Metadata'
    description = 'Downloads metadata and covers from DriveThruCards.'
    ...
    touched_fields = frozenset(['title', 'authors', 'tags', 'pubdate', 'comments', 'publisher', 'identifier:drivethrucards', 'series', 'rating'])
    ...
    def get_base_url(self):
        return 'https://www.drivethrucards.com'
    ...
    def _fetch_url(self, url, timeout=30):
        ...
        # Select cookie
        cookie = self.prefs.get('cf_cookie_dtcards')
    ...
    def identify(self, log, result_queue, abort, title=None, authors=None, identifiers={}, timeout=30):
        ...
        plugin_id = identifiers.get('drivethrucards', None)
    ...
    def _api_search(self, query, timeout, log):
        import json
        # api.drivethrucards.com/api/vBeta/search_ahead?keyword=...
        base = "https://api.drivethrucards.com/api/vBeta/search_ahead"
        
        # Site ID: 73, Group ID: 26
        site_id = 73
        group_id = 26
    ...
    def _get_product_details(self, product_id, result_queue, log, timeout):
        ...
        url = f"https://api.drivethrucards.com/api/vBeta/products/{product_id}"
        ...
        mi.set_identifier('drivethrucards', str(product_id))
        mi.source = self.name
        ...
        if img_path:
            # Construct full URL
            cover_url = f"https://www.drivethrucards.com/images/{img_path}"
    ...
    def get_cached_cover_url(self, identifiers):
        ...
        plugin_id = identifiers.get('drivethrucards')
        if plugin_id:
...
if __name__ == '__main__':
    ...
    import os
    if os.path.exists('../cookie_dtcards.txt'):
        with open('../cookie_dtcards.txt', 'r') as f:
            cookie = f.read().strip()
            print(f"Using DTCards cookie from file: {cookie[:10]}...")
            DriveThruCardsSource.prefs['cf_cookie_dtcards'] = cookie
            
    print("Running DriveThruCards tests...")
    tests = [
        (
            {'identifiers': {},
             'title': 'The Great Book of Random Tables', 'authors': ['Matt Davids']},
             # Likely won't find this exact book on cards site, but just structural test
            [title_test('Random Tables', exact=False)]
        )
    ]
    
    test_identify_plugin(DriveThruCardsSource.name, tests)
