...
class ConfigWidget(QWidget):
    def __init__(self, plugin):
        ...
        # Cookie Input
        self.cookie_label = QLabel('Wargame Vault "cf_clearance" Cookie:', self)
        layout.addWidget(self.cookie_label)
        
        self.cookie_input = QLineEdit(self)
        self.cookie_input.setText(plugin.prefs.get('cf_cookie_wgv', ''))
        self.cookie_input.setPlaceholderText('Paste Wargame Vault cookie here')
        layout.addWidget(self.cookie_input)

        layout.addStretch(1)

    def save_settings(self):
        self.plugin.prefs['cf_cookie_wgv'] = self.cookie_input.text().strip()

class WargameVaultSource(Source):
    name = 'Wargame Vault Metadata'
    description = 'Downloads metadata and covers from Wargame Vault.'
    ...
    touched_fields = frozenset(['title', 'authors', 'tags', 'pubdate', 'comments', 'publisher', 'identifier:wargamevault', 'series', 'rating'])
    ...
    def get_base_url(self):
        return 'https://www.wargamevault.com'
    ...
    def _fetch_url(self, url, timeout=30):
        ...
        # Select cookie
        cookie = self.prefs.get('cf_cookie_wgv')
    ...
    def identify(self, log, result_queue, abort, title=None, authors=None, identifiers={}, timeout=30):
        ...
        plugin_id = identifiers.get('wargamevault', None)
    ...
    def _api_search(self, query, timeout, log):
        import json
        # api.wargamevault.com/api/vBeta/search_ahead?keyword=...
        base = "https://api.wargamevault.com/api/vBeta/search_ahead"
        
        # Site ID: 56, Group ID: 19
        site_id = 56
        group_id = 19
    ...
    def _get_product_details(self, product_id, result_queue, log, timeout):
        ...
        url = f"https://api.wargamevault.com/api/vBeta/products/{product_id}"
        ...
        mi.set_identifier('wargamevault', str(product_id))
        mi.source = self.name
        ...
        if img_path:
            # Construct full URL
            cover_url = f"https://www.wargamevault.com/images/{img_path}"
    ...
    def get_cached_cover_url(self, identifiers):
        ...
        plugin_id = identifiers.get('wargamevault')
        if plugin_id:
...
if __name__ == '__main__':
    ...
    import os
    if os.path.exists('../cookie_wgv.txt'):
        with open('../cookie_wgv.txt', 'r') as f:
            cookie = f.read().strip()
            print(f"Using WGV cookie from file: {cookie[:10]}...")
            WargameVaultSource.prefs['cf_cookie_wgv'] = cookie
            
    print("Running Wargame Vault tests...")
    tests = [
        (
            {'identifiers': {},
             'title': 'Warhammer', 'authors': ['Games Workshop']},
             # Structural test
            [title_test('Warhammer', exact=False)]
        )
    ]
    
    test_identify_plugin(WargameVaultSource.name, tests)
